...
### ✅ 已修复:把 q 维切块做进 indexer 内部

参照 SparseGQA(prefill 固定 `CHUNK=4096`,把 q 维钉成常数)的做法,在
`fp4_indexer_block_scores` 内部按 `q_chunk` 分段调用,默认 **8192**(实测最优,见下)。

**实现上没有拷贝、没有指针运算。** kernel 的写出索引是
`q_global_store = q_begin + q_local`,而 `q_begin = mCuQ[batch_idx]` ——
所以只要传**绝对** `cu_seqlens_q = [lo, hi)`,一个 chunk 的结果就直接落进全量 score
张量的正确列;`max_k_tiles` 与 `total_q` 保持全量(它们只决定 layout 的 stride 和
`ktile` 上界),causal 范围靠 `cu_seqlens_k = [0, hi + causal_offset)` 表达,
与不切块时该 chunk 的因果范围完全一致。**输出逐位相同**(已验证)。

chunk 大小实测(causal,Hkv=4/D=128):

| chunk | 32K | 16K | **8K** | 4K | 2K | 不切 |
|---|---|---|---|---|---|---|
| 128K | 11.19 | 8.60 | **7.39** | 9.17 | 13.08 | 35.09 ms |
| 256K | 42.12 | 24.22 | **23.98** | 27.88 | 35.98 | 257.68 ms |

8192 在两档都是最低点:更大则扫描链仍长,更小则启动次数和 K 重读的代价上来。
(SparseGQA 取 4096,同样的道理。)

**修复后 indexer 单级:**

| S | 修复前 | 修复后 | 加速 | 修复前 TFLOPS | 修复后 TFLOPS |
|---|---|---|---|---|---|
| 32K | 0.84 ms | 0.55 | 1.53× | 654 | 992 |
| 64K | 5.08 | 1.87 | 2.72× | 433 | 1175 |
| 128K | 35.08 | 7.08 | 4.95× | 251 | 1243 |
| 256K | 257.69 | **27.22** | **9.47×** | 137 | **1293** |

伸缩性回到二次(比值 3.38 → 3.78 → 3.85,理论 4.00),效率**平且随规模变好**。
对照 FP8 路径的 1012 TFLOPS,FP4 现在**每 flop 快 1.28 倍、墙钟快 2.55 倍**
(它只做一半 flops)——这才是精度差应有的排序。
...
曲线由「64K 见顶后塌陷」变为**随上下文单调上升**。§1 说的
「稀疏省下的时间正被打分器吃掉」到此为止。

### 仍可做的

1. **闭式求逆(更彻底)**。`task_idx → q_tile` 的累积计数约 `i²/32`,可用 `sqrt` 加一两步
   牛顿迭代 O(1) 求出,彻底删掉 while 循环,连切块都不需要。切块把扫描链从
   `S/128` 压到 `8192/128 = 64`,但没有消除它。
...
## 24. ⚠️ cutlass-dsl 有一条硬分界：4.5.x 能跑 decode，4.6+ 的 FA4 快 3.3×

§23 为解除 decode 阻塞把容器降到 4.5.1，并称「prefill 侧零回归」。
**那句话只验了正确性没测性能，是错的。** 扫遍可用版本后，分界线很干净：

| cutlass-dsl | decode 内核 | FA4 @128K |
|---|---|---|
| 4.5.1 | ✅ 2/2 通过 | 303.9 ms |
| 4.5.3 | ✅ 2/2 通过 | 318.5 ms |
| **4.6.0** | ❌ CLC 验证失败 | **92.0 ms** |
| 4.6.2 | ❌ 同样失败 | 92.2 ms |
| 4.7.0 | ❌ 同样失败 | 92.2 ms |

**没有兼得的版本**，升到最新的 4.7.0 也没修好——说明要改的是仓库这边
（`fwd_decode/tile_scheduler.py` 里 `cute.arch.clc_response` 的用法），不是等上游。

**不是测量噪声也不是 GPU 干扰**：92 ms 在 4.6.0/4.6.2/4.7.0 三个版本上重复
（92.04 / 92.21 / 92.22），303–318 ms 在两个 4.5.x 上重复；测量全程在空闲 GPU 上。

> 一个未拆干净的变量：4.5.x 必须配 quack 0.4.x（0.6.x 用了 4.6.0 才有的
> `cpasync.ReductionKind`），所以严格说是这个**组合**导致 FA4 慢。不过 FA4 走
> `flash_attn.cute`、不经过 quack，所以更可能是 cutlass 本身。

### 结论：系统保持 4.6.0

FA4 是所有加速比的分母，4.5.x 会让整张速度表虚高约 3.3×。decode 用独立
venv（4.5.1 + quack 0.4.1）跑，直到 CLC 路径改好为止。**任何 decode 数字必须注明
是在 4.5.x 下测的，任何 prefill 分母必须在 4.6.0 下测**，否则两者不可比。

> 教训：版本切换后「跑一遍正确性」不等于「没有回归」。
> 性能回归不会让任何断言失败，只会让所有比值悄悄变好看。
