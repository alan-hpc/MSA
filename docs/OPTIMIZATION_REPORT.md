...
| 17 | **dense FMHA 在 512K prefill 非法访存**（Q/O 元素数正好 2³¹），且不总是崩 | 污染 CUDA context 带走整场扫描；**512K 列的分母作废** | ✅ 已加守卫 |
...
**因此速度表 512K 那一列的分母是在已污染状态下侥幸跑出来的，不可信，应当作废。**
`log.html` 与本报告的 512K 列已相应标注。
