...
## 24. ⚠️ cutlass-dsl 版本是 prefill 与 decode 的取舍，不能二选一装

§23 为解除 decode 阻塞把容器降到 cutlass-dsl 4.5.1，并称「prefill 侧零回归」。
**那句话只验了正确性，没重测性能，是错的。**

| | 4.6.0 | 4.5.1 |
|---|---|---|
| 仓库 dense @256K | 423.3 ms | 423.7 ms（不变） |
| **FA4 @256K** | **369.7 ms** | **1214.2 ms（慢 3.2×）** |
| baseline 全流水线 @256K | 56.9 ms | 58.2 ms（基本不变） |
| decode 内核 | **构建失败** | 正常，4/4 通过 |

**只有 FA4 退化**，而它正是所有加速比的分母——4.5.1 下整张速度表会系统性虚高约 3 倍。

### 结论：分开装

- **系统环境保持 4.6.0**：prefill 扫描、FA4 分母、quack 依赖的路径都在这里。
- **decode 走独立 venv（4.5.1 + quack 0.4.1）**：`§16` 记的那套。
  两个包必须配套——`quack 0.6.x` 用了 4.6.0 才有的 `cpasync.ReductionKind`。

真正的解法是修 `fwd_decode/tile_scheduler.py` 的 CLC 路径以通过 4.6.0 的 MLIR 验证，
那样两条路径能共存于一个环境。在此之前，**任何 decode 数字都必须注明是在 4.5.1 下测的，
任何 prefill 分母都必须在 4.6.0 下测**，否则两者不可比。
