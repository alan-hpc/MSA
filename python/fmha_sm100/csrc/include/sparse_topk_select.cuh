...
__global__ void __launch_bounds__(kTransposeTile* kTransposeBlockRows)
    SparseTopKTransposeKernel(const float* __restrict__ in, float* __restrict__ out, uint32_t K,
                              ...
                              // pins some other request's last page and never
                              // pins its own.
                              uint32_t force_end,
  ...
  const float* in_head = in + static_cast<size_t>(head) * group_size * head_stride;
...
#pragma unroll
  for (int dq = 0; dq < kTransposeTile; dq += kTransposeBlockRows) {
    ...
    if (q_store < qo && k_store < K) {
      uint32_t row_end = num_valid_pages;
      if (per_row_valid != nullptr) {
        const int32_t v = per_row_valid[q_store];
        if (v > 0 && static_cast<uint32_t>(v) < row_end) row_end = v;
      }
      const uint32_t force_start = (force_end < row_end) ? row_end - force_end : 0;
      const bool is_forced = (k_store < force_begin) ||
                             (force_end > 0 && k_store >= force_start && k_store < row_end);
      out_head[static_cast<size_t>(q_store) * K + k_store] =
          is_forced ? FLT_MAX : tile[tx][ty + dq] * prescale;
    }
  }
}
...
__global__ void __launch_bounds__(TILE * TILE / 4) SparseTopKTransposeXorF4Kernel(
    ...
    uint32_t force_begin, uint32_t num_valid_pages,
    ...
    float prescale,
    // v2.8_group_max_in_transpose: a shared-route checkpoint ranks pages on the
    // max over its index-head group. Reducing before the call costs a full pass
  ...
  const size_t head_stride = static_cast<size_t>(K) * qo;
  const float* in_head  = in  + static_cast<size_t>(h) * group_size * head_stride;
  ...
  // ---- STORE: scalar swizzled SMEM reads → float4 GMEM -----------------
  {
    ...
    uint32_t row_end = num_valid_pages;
    if (per_row_valid != nullptr) {
      const int32_t rv = per_row_valid[q_out];
      if (rv > 0 && static_cast<uint32_t>(rv) < row_end) row_end = rv;
    }
    const uint32_t force_start = (force_end < row_end) ? row_end - force_end : 0;
    auto is_forced = [&](uint32_t k) -> bool {
      return (k < force_begin) ||
             (force_end > 0 && k >= force_start && k < row_end);
    };
...
cudaError_t LaunchTransposeAndIndexerTopK(const float* in_strided, float* transposed, int32_t* out,
  ...
  //          otherwise fall back to the original padded-tile transpose.
  {
    ...
    const bool xor_path_ok = (total_qo_len % kXorTile == 0) &&
                             ...
                             (max_k_tiles % 4 == 0);
    if (xor_path_ok) {
      dim3 grid(total_qo_len / kXorTile, max_k_tiles / kXorTile, out_heads);
      ...
      dim3 grid((total_qo_len + kTransposeTile - 1) / kTransposeTile,
                (max_k_tiles + kTransposeTile - 1) / kTransposeTile, out_heads);
  ...
  // ---- (2) IndexerTopK + fused sort + write to qo_outermost layout -------
  {
    ...
    void* args[] = {(void*)&transposed, (void*)&out, (void*)&total_qo_len,
                    (void*)&out_heads, (void*)&max_k_tiles, (void*)&topk,
...
inline size_t SparseTopKWorkspaceSize(uint32_t total_qo_len, uint32_t num_qo_heads,
                                      uint32_t max_k_tiles, uint32_t group_size = 1) {
  // The transpose writes one row per *reduced* head, so the buffer it needs
  // shrinks with the group fold -- the input rows are read, never staged.
  return static_cast<size_t>(num_qo_heads / group_size) * max_k_tiles * total_qo_len;
}
...
inline cudaError_t SparseTopKSelect(const float* in, int32_t* out, int32_t* workspace,
  ...
  if (max_k_tiles <= topk) {
    dim3 grid(num_rows);
    dim3 block(topk);
    void* args[] = {(void*)&out, (void*)&total_qo_len, (void*)&out_heads,
